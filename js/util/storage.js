...
export function setBankAddInterceptor(fn) {
...
export function watchStorageKey(key, { parse, equals, onChange, emitCurrentValue = false } = {}) {
...
export function updateLastSaveTime() {
    ...
    try {
        lsSetItem(getLastSaveTimeKey(slot), String(Math.floor(saveTime)));
        if (wasInterrupted) {
            lsSetItem(`ccc:simInterrupted:${slot}`, "1");
        } else {
...
export function setActiveSlot(n) {
    ...
    _activeSlotCache = v;
    lsSetItem(KEYS.SAVE_SLOT, String(v));
    try {
        lsSetItem(`ccc:instanceId:${v}`, _myInstanceId);
    } catch {}
    try {
...
export function clearActiveSlot() {
    ...
    if (currentSlot != null && hasEnteredGameSession) {
        try {
            memoryLastSaveTime = Date.now();
            lsSetItem(getLastSaveTimeKey(currentSlot), String(memoryLastSaveTime));
        } catch (e) {
            console.error(e);
        }
    }
...
export function setSlotSignature(slot, signature) {
    ...
    try {
        if (signature == null) {
            ...
            lsSetItem(key, String(signature));
        }
    } catch {}
}
...
export function unmarkSaveSlotModified(slot = getActiveSlot()) {
    ...
    try {
        ...
        window.dispatchEvent(
            new CustomEvent("saveIntegrity:storageMutation", { detail: { slot: normalized, trusted: true } }),
        );
...
export function markSaveSlotModified(slot = getActiveSlot()) {
    ...
    try {
        lsSetItem(key, "1");
        console.warn(
            `[Integrity] Save Slot ${normalized} has been permanently marked as modified due to debug panel usage or manual data tampering.`,
        );
    } catch {
        return;
    }
...
export function setSavedArea(areaID, slot = getActiveSlot()) {
    ...
    lsSetItem(`${KEYS.CURRENT_AREA}:${slot}`, areaID);
}
...
export function incrementResetStat(resetName) {
    try {
        ...
        if (!list.includes(resetName)) {
            list.push(resetName);
            lsSetItem(listKey, JSON.stringify(list));
        }
    } catch {}
}
