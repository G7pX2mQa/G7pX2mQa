...
export function isManageMode() { return currentMode !== null && currentMode !== 'menu'; }
...
function setManageMode(mode) {
  ...
  if (mode === 'menu') {
    ...
    if (titleEl) {
      titleEl.textContent = isExport ? "Select a save slot to export" : "Select a save slot to import onto (warning: will overwrite data)";
...
function showCancelButton() {
  if (!cancelBtnContainer) {
    ...
    btn.onmouseenter = () => { btn.style.background = 'hsla(0, 80%, 40%, 0.95)'; };
    btn.onmouseleave = () => { btn.style.background = 'hsla(0, 80%, 40%, 0.85)'; btn.style.transform = ''; };
    btn.onmousedown = () => { btn.style.transform = 'translateY(1px)'; btn.style.background = 'hsla(0, 80%, 40%, 1.0)'; };
    btn.onmouseup = () => { btn.style.transform = ''; btn.style.background = 'hsla(0, 80%, 40%, 0.95)'; };
...
function applySaveDataToSlot(slot, data) {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (key.endsWith(`:${slot}`)) {
      ...
      localStorage.setItem(`${baseKey}:${slot}`, value);
    }
  }
