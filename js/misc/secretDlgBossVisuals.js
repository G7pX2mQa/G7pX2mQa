...
export function playSecretDlgBossFightSequence(container, onComplete, options = {}) {
    ...
    window.secretBossHpForTesting = (hp) => {
        if (hp !== undefined) {
            bossHp = hp;
            updateBossHpUI();
        }
    ...
    function getSandHeight(globalX, layerBaseY, layerAmplitude, layerPeriod, layerSeed) {
        let yOffset =
            Math.sin(globalX / layerPeriod + layerSeed) * layerAmplitude +
    ...
    function playPlayerDeathSequence() {
        ...
        class Particle {
            constructor(x, y, isLong, customAngle = null, customSpeed = null, customSize = null) {
                ...
                const speed =
                    customSpeed !== null ? customSpeed : isLong ? Math.random() * 20 + 5 : Math.random() * 10 + 2;
    ...
    function loop(timestamp) {
                ...
                if (!isNaN(duration) && duration > 0) {
                    if (timeSinceDeath >= 9000 + duration) {
                        ...
                        if (fadeProgress >= 1.0 && timeSinceDeath >= 9000 + duration + 6000 && !victoryUIAdded) {
                            ...
                            const styleEl = document.createElement("style");
                            styleEl.textContent =
                                "#boss-victory-container, #boss-victory-container * { cursor: none !important; }";
                            ...
                            title.style.textShadow =
                                "0 4px 8px rgba(0,0,0,0.8), -2px -2px 0 #000, 2px -2px 0 #000, -2px 2px 0 #000, 2px 2px 0 #000";
                            ...
                            stats.style.textShadow =
                                "0 2px 4px rgba(0,0,0,0.8), -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000";
                ...
                function drawSandAndPropsToCtx(targetCtx) {
                    ...
                    layers.forEach((layer) => {
                        ...
                        for (let x = 0; x <= width + step; x += step) {
                            ...
                            targetCtx.lineTo(
                                x,
                                getSandHeight(globalX, layer.baseY, layer.amplitude, layer.period, layer.seed),
                            );
                        }
        ...
        if (
            timestamp - lastSpawnTime >= effectiveSpawnInterval &&
            currentBossWidth > 0 &&
            !isBossDead &&
            !isPlayerDead
        ) {
            ...
            if (cursorScreenX !== null && cursorScreenY !== null) {
                const rxLeft = currentBossWidth * (cursorScreenX < leftEyeScreenX ? 0.1 : 0.05);
                ...
                const rxRight = currentBossWidth * (cursorScreenX > rightEyeScreenX ? 0.1 : 0.05);
            ...
            if (
                bossHp <= 500 &&
                !rubyCoinSpawned &&
                (Math.random() < 0.01 * (effectiveSpawnInterval / (1000 / 60)) || window.forceRubyCoinTest)
            ) {
                projType = "rubyCoin";
                ...
                Math.random() < 0.0005 * (effectiveSpawnInterval / (1000 / 60))
            ) {
                projType = "life";
            ...
            let decelRatio = Math.random() < 0.75 ? 0.6 : Math.random() * 0.5 + 0.1;
        ...
        for (let i = activeBombColumns.length - 1; i >= 0; i--) {
            ...
            if (col.state === "constructing") {
                ...
                while (
                    col.bombsConstructed < col.numBombs &&
                    timestamp - col.lastBombSpawnTime >= effectiveTimePerBomb
                ) {
        ...
        if (rubyCoinRef && activeProjectiles.includes(rubyCoinRef)) {
            ...
            if (true) {
                ...
                if (rubyCoinActive && rubyCoinSwipes > 0) {
                    ...
                    let popScale = 1.0;
                    const timeSinceSwipe = performance.now() - rubyCoinLastSwipeTime;
                    if (timeSinceSwipe < 200) {
                        popScale = 1.0 + Math.sin((timeSinceSwipe / 200) * Math.PI) * 0.2;
                    }
    ...
    function updateMusicSpeed() {
        ...
        if (rubyCoinActive) {
            ...
            const rates = [1.0, 1.01, 1.02, 1.03, 1.04, 1.065, 1.09, 1.115, 1.14, 1.2];
    ...
    function onBossCursorHit(e) {
        ...
        for (let i = 0; i < activeBombColumns.length; i++) {
            ...
            if (col.state === "moving" || col.state === "pausing") {
                ...
                const passedThroughX =
                    (lastCx <= maxX && cx >= minX) || (lastCx >= minX && cx <= maxX) || (cx >= minX && cx <= maxX);
                if (passedThroughX) {
