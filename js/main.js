...
function applyPendingSlotWipe() {
    ...
    try {
        ...
        try {
            lsRemoveItem("ccc:pendingSlotWipe");
        } catch {}
    } catch {
        try {
            lsRemoveItem("ccc:pendingSlotWipe");
...
function finishAndHideLoader(loaderEl, onFadeStart, finishedText, dwellMs = 500) {
    ...
    if (loaderEl.__label) {
        if (finishedText) {
            ...
            loaderEl.__label.textContent = loaderEl.__skipped ? "Loading skipped" : "Finished loading assets";
        }
    }
...
async function preloadAssetsWithProgress({ images = [], audio = [], fonts = true }, onProgress) {
    ...
    const tasks = [...preloadImages(images, bump), ...preloadAudio(audio, bump), ...(fonts ? preloadFonts(bump) : [])];
...
export function enterArea(areaID, fadeDuration = 0) {
    ...
    if (areaID === AREAS.JAIL) {
        ...
        Array.from(document.body.children).forEach((child) => {
            if (
                child.id !== "jail-screen" &&
                child.tagName !== "SCRIPT" &&
                !child.classList.contains("debug-panel") &&
                child.id !== "debug-panel-toggle-button" &&
                child.className !== "debug-panel-toggle-button"
            ) {
    ...
    if (!spawner) {
        ...
        const applyMutationSprite = () => {
            ...
            try {
                spawner.setCoinSprite(getMutationCoinSprite?.());
            } catch {}
        };
        applyMutationSprite();
    ...
    try {
        window.dispatchEvent(
            new CustomEvent("menu:visibilitychange", {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const ASSET_MANIFEST = {
        images: [
			...
			"img/currencies/coral/coral_base_v1.webp",
			"img/currencies/coral/coral_base_v1_plus_coral_red.webp",
			...
			"img/currencies/rubble/rubble.webp",
			"img/currencies/rubble/rubble_base.webp",
			"img/currencies/rubble/rubble_plus_base.webp",
			...
			"img/lab_icons/pp_val0.webp",
			"img/lab_icons/scrap_val0.webp",
            "img/lab_icons/tsunami_exponent.webp",
			...
			"img/misc/prismatic_pickaxe.webp",
			"img/misc/rubble_value.webp",
			"img/misc/sad_merchant.webp",
        ...
        audio: [
			...
			"sounds/explosion_caused_by_spinny.ogg",
    ...
    ({
        ...
        onMutationChange: onMutationChangeGame,
        getMutationState,
    } = mutationModule);
    ({ initPopups } = popupModule);
    ({
        installsuspensionSafeguards,
        restoreFromBackupIfNeeded: restoreSuspendBackup,
        markProgressDirty,
        flushBackupSnapshot,
    } = safetyModule);
    ...
    try {
        ...
        console.error("Surge effects init failed", e);
    }
    try {
        initLabLogic();
    } catch (e) {
        console.error("Lab system init failed", e);
    }
    try {
        initFpsTracker();
    } catch (e) {
        console.error("FPS tracker init failed", e);
    }
    try {
        initPerformanceGraph();
    } catch (e) {
        console.error("Performance graph init failed", e);
    }
    try {
        initFlowSystem();
    } catch (e) {
        console.error("Flow system init failed", e);
    }
...
⚠️ Note:
ANY modification of stats, currencies, upgrade levels, or other save data through the debug panel will permanently mark the save slot as modified. If the save slot is marked as modified, its shop button will permanently turn from a fresh green to a poopy brown color, which I like to call the poop-shop of shame.
