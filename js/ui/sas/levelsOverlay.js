...
function createLevelRow(container, isUniversal, levelConfig, progConfig, prefix) {
    ...
    if (prefix && prefix !== "universal") {
        ...
        if (progConfig && progConfig.bgGradient) {
            row.style.background = progConfig.bgGradient;
        }
    }
    ...
    if (isUniversal) {
        ...
        if (levelConfig.barText) {
            ...
            setHtmlOrText(
                levelDiv,
                levelConfig.barText.replace("{val}", formatNumber(0)).replace("<span ", "<span data-level-val "),
            );
    ...
    if (isUniversal && !IS_MOBILE) {
        opts.push({
            value: "paintbrush",
            label: "Enable Multi-Toggle",
            isButton: true,
            className: "paintbrush-btn-anim",
        });
    }
    ...
    const setDropdownValue = (newVals) => {
        ...
        const toggledType = ["popups", "pinned"].find((type) => prevVals.includes(type) !== newVals.includes(type));
    ...
    levels.forEach((l) => {
        ...
        if (row) {
            ...
            if (l.progConfig) {
                ...
                if (
                    reqVal === Infinity ||
                    (typeof reqVal === "string" && reqVal === "Infinity") ||
                    (reqVal && typeof reqVal.isInfinite === "function" && reqVal.isInfinite())
                ) {
                ...
                if (suffixEl && l.progConfig) {
                    ...
                    setHtmlOrText(
                        suffixEl,
                        l.progConfig.plural && reqStr !== "1" ? l.progConfig.plural : l.progConfig.singular,
                    );
                }
