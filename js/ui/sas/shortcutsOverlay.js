...
function populateShortcutsOverlay(overlayEl) {
    ...
    let numberKeyDesc =
        "Inside Delve overlays, press number keys 0 through 9 to instantly swap between tabs. First tab is 0, second tab is 1, etc. and 9 is last unlocked tab.";
    if (typeof isFlowUnlocked === "function" && isFlowUnlocked()) {
        numberKeyDesc +=
            ' For the Flow tab specifically, press "F" to toggle Waterwheel Hotkey mode, where 0 is Coin Waterwheel, 1 is XP Waterwheel, etc. and 9 is last unlocked Waterwheel.';
    }
    ...
    const shortcuts = [
        ...
        {
            key: "Shift+LC",
            desc: "On any sort of Shop upgrade, hold shift and left-click its icon to perform a Buy Cheap onto it. Upgrades that do not support this will default to Buy Max.",
        },
    ];
    if (typeof hasDoneForgeReset === "function" && hasDoneForgeReset()) {
        shortcuts.push({
            key: "Ctrl+LC",
            desc: "On any sort of Shop upgrade, hold ctrl and left-click its icon to perform a Buy Next onto it. Upgrades that do not support this will default to Buy Max.",
        });
    }
    shortcuts.push(
    ...
    if (isBossBeaten) {
        shortcuts.push({
            id: "r",
            key: "R",
            desc: "In the secret Merchant boss fight, press R to restart the fight immediately.",
        });
    }
    ...
    window.addEventListener("unlock:change", (e) => {
        ...
        if (e.detail && e.detail.key === "flow") {
            ...
            if (overlayEl) {
                ...
                if (numKeyDescEl) {
                    let numberKeyDesc =
                        "Inside Delve overlays, press number keys 0 through 9 to instantly swap between tabs. First tab is 0, second tab is 1, etc. and pressing 9 sends to last unlocked tab.";
                    if (typeof isFlowUnlocked === "function" && isFlowUnlocked()) {
                        numberKeyDesc +=
                            ' For the Flow tab specifically, press "F" to toggle Waterwheel Hotkey mode, where 0 is Coin Waterwheel, 1 is XP Waterwheel, etc. and 9 is last unlocked Waterwheel.';
                    }
