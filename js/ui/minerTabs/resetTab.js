...
export function setCombineUnlocked(value, slot = getActiveSlot()) {
    ...
    if (typeof localStorage !== "undefined") {
        try {
            if (value) {
                lsSetItem(`${COMBINE_UNLOCKED_KEY_BASE}:${slotKey}`, "1");
            } else {
                lsRemoveItem(`${COMBINE_UNLOCKED_KEY_BASE}:${slotKey}`);
            }
        } catch {}
    }
    ...
    updateCombineCard();
    try {
        window.dispatchEvent(new CustomEvent("unlock:change", { detail: { key: "combine", slot } }));
    } catch {}
}
...
export function setCombineResetCompleted(value, slot = getActiveSlot()) {
    ...
    if (typeof localStorage !== "undefined") {
        try {
            if (value) {
                lsSetItem(`${COMBINE_COMPLETED_KEY_BASE}:${slotKey}`, "1");
            } else {
                lsRemoveItem(`${COMBINE_COMPLETED_KEY_BASE}:${slotKey}`);
            }
        } catch {}
    }
...
export function setCompressUnlocked(value, slot = getActiveSlot()) {
    ...
    if (typeof localStorage !== "undefined") {
        try {
            if (value) {
                lsSetItem(`${COMPRESS_UNLOCKED_KEY_BASE}:${slotKey}`, "1");
            } else {
                lsRemoveItem(`${COMPRESS_UNLOCKED_KEY_BASE}:${slotKey}`);
            }
        } catch {}
    }
    ...
    updateCompressCard();
    try {
        window.dispatchEvent(new CustomEvent("unlock:change", { detail: { key: "compress", slot } }));
    } catch {}
}
...
export function setCompressResetCompleted(value, slot = getActiveSlot()) {
    ...
    if (typeof localStorage !== "undefined") {
        try {
            if (value) {
                lsSetItem(`${COMPRESS_COMPLETED_KEY_BASE}:${slotKey}`, "1");
            } else {
                lsRemoveItem(`${COMPRESS_COMPLETED_KEY_BASE}:${slotKey}`);
            }
        } catch {}
    }
...
export function performCollapseReset(slot = getActiveSlot()) {
    ...
    // Forcefully sync coin multiplier so that post-reset surge level 200 and challenge debuffs apply immediately.
    try {
        syncCoinMultiplierWithXpLevel(true);
    } catch {}
...
function updateCombineCard() {
    ...
    if (!el.card.style.display || el.card.style.display === "none") {
    ...
    if (!checkCombineRequirements()) {
        updateResetButtonContent(el.btn, {
            disabled: true,
            msg: "Get at least 3 unique Materials to perform a Combine reset",
        });
        return;
    }
