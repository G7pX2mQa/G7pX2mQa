...
export function setSellTabViewed(value, slot = getActiveSlot()) {
    ...
    if (typeof localStorage !== "undefined") {
        try {
            if (value) {
                lsSetItem(`${SELL_VIEWED_KEY_BASE}:${slotKey}`, "1");
            } else {
                lsRemoveItem(`${SELL_VIEWED_KEY_BASE}:${slotKey}`);
            }
...
export function isSellUnlocked() {
    ...
    if (cachedSellUnlockedStates[slot] !== undefined && cachedSellUnlockedStates[slot] !== null)
        return cachedSellUnlockedStates[slot];
...
export function setSellUnlocked(value, slot = getActiveSlot()) {
    ...
    if (typeof localStorage !== "undefined") {
        try {
            if (value) {
                lsSetItem(`${SELL_UNLOCKED_KEY_BASE}:${slotKey}`, "1");
            } else {
                lsRemoveItem(`${SELL_UNLOCKED_KEY_BASE}:${slotKey}`);
            }
        } catch {}
    }
}
...
${clickWord} the button below to toggle whether you sell Materials for Scrap or Rubble`.trim();
...
function createSellRow(matKey, index) {
                ...
                if (res !== null && res.trim() !== "") {
                    ...
                    if (parsed) {
                        ...
                        if (rowCache) {
                            ...
                            const slot = getActiveSlot() ?? "default";
                            lsSetItem(`ccc:sellAmount:${slot}:${matKey}`, newAmount);
                        }
                    }
                }
            } else {
    ...
    sellBtn.addEventListener("click", () => {
        ...
        if (amt.cmp(5) >= 0) {
            ...
            try {
                parsedNum =
                    amt.inf || amt.e >= BigNum.DEFAULT_PRECISION ? Infinity : Number(amt.toPlainIntegerString());
            } catch (e) {
                parsedNum = 5;
            }
        }
