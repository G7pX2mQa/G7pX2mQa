...
import { getXpState, resetXpProgress, onXpChange } from "../../game/xpSystem.js";
import {
    AREA_KEYS,
    UPGRADE_TIES,
    getUpgradesForArea,
...
import {
    showDelayedSecretAchievementNotifications,
...
import { getPendingCores, getPendingCrystals } from "../minerTabs/resetTab.js";
const BN = BigNum;
...
function getPendingGoldWithMultiplier(multiplierOverride = null) {
    try {
        ...
        if (multiplierOverride) {
            const mult =
                multiplierOverride instanceof BN ? multiplierOverride : BigNum.fromAny(multiplierOverride ?? 1);
...
function computeForgeGold(coinsBn, levelBn) {
    ...
    const pow14 = levelFactor <= 0 ? bnOne() : bigNumFromLog10(levelFactor * Math.log10(1.4));
    const floorLog = Math.floor(logScaled);
    const pow115 = floorLog <= 0 ? bnOne() : bigNumFromLog10(floorLog * Math.log10(1.15));
...
function computeInfuseMagicBase(coinsBn, cumulativeMpBn) {
    ...
    const LOG_2 = 0.30103; // Math.log10(2)
    let logMpRatio = 0;
...
function computeSurgeWaves(xpLevelBn, coinsBn, goldBn, magicBn, mpBn) {
    ...
    // Handle Infinity
    if (
        logCoins === Number.POSITIVE_INFINITY ||
...
export function setForgeResetCompleted(value) {
    ...
    try {
        lsSetItem(FORGE_COMPLETED_KEY(slot), resetState.hasDoneForgeReset ? "1" : "0");
    } catch {}
    try {
        window.dispatchEvent(new CustomEvent("forge:completed", { detail: { slot } }));
    } catch {}
}
...
export function setInfuseResetCompleted(value) {
    ...
    try {
        lsSetItem(INFUSE_COMPLETED_KEY(slot), resetState.hasDoneInfuseReset ? "1" : "0");
    } catch {}
    try {
        window.dispatchEvent(new CustomEvent("unlock:change", { detail: { key: "infuse", slot } }));
    } catch {}
...
export function setSurgeResetCompleted(value) {
    ...
    try {
        lsSetItem(SURGE_COMPLETED_KEY(slot), resetState.hasDoneSurgeReset ? "1" : "0");
    } catch {}
...
export function setExperimentResetCompleted(value) {
    ...
    try {
        lsSetItem(EXPERIMENT_COMPLETED_KEY(slot), resetState.hasDoneExperimentReset ? "1" : "0");
    } catch {}
    try {
        window.dispatchEvent(new CustomEvent("unlock:change", { detail: { key: "experiment_completed", slot } }));
    } catch {}
}
...
export function setInfuseUnlocked(value) {
    ...
    try {
        lsSetItem(INFUSE_UNLOCK_KEY(slot), resetState.infuseUnlocked ? "1" : "0");
    } catch {}
...
export function setForgeUnlocked(value) {
    ...
    try {
        lsSetItem(FORGE_UNLOCK_KEY(slot), resetState.forgeUnlocked ? "1" : "0");
    } catch {}
...
export function setSurgeUnlocked(value) {
    ...
    try {
        lsSetItem(SURGE_UNLOCK_KEY(slot), resetState.surgeUnlocked ? "1" : "0");
    } catch {}
...
function bindStorageWatchers(slot) {
    ...
    if (slot == null) return;
    watchers.push(
        watchStorageKey(FORGE_UNLOCK_KEY(slot), {
            onChange(value) {
                const next = value === "1";
    ...
    watchers.push(
        watchStorageKey(FORGE_COMPLETED_KEY(slot), {
            onChange(value) {
                const next = value === "1";
    ...
    watchers.push(
        watchStorageKey(INFUSE_COMPLETED_KEY(slot), {
            onChange(value) {
                const next = value === "1";
    ...
    watchers.push(
        watchStorageKey(SURGE_COMPLETED_KEY(slot), {
            onChange(value) {
                const next = value === "1";
    ...
    watchers.push(
        watchStorageKey(EXPERIMENT_COMPLETED_KEY(slot), {
            onChange(value) {
                const next = value === "1";
...
function recomputePendingDna() {
    ...
    try {
        ...
        if (bank.dna?.mult?.applyTo) {
            result = bank.dna.mult.applyTo(result);
        } else if (bank.DNA?.mult?.applyTo) {
            result = bank.DNA.mult.applyTo(result);
        }
...
export function performForgeReset() {
    ...
    if (settingsManager.get("forge_confirmation")) {
        if (!window.confirm("Are you sure you want to perform a Forge reset?")) return false;
    }
...
export function performInfuseReset() {
    ...
    if (settingsManager.get("infuse_confirmation")) {
        if (!window.confirm("Are you sure you want to perform an Infuse reset?")) return false;
    }
...
function performExperimentReset() {
    ...
    if (settingsManager.get("experiment_confirmation")) {
        if (!window.confirm("Are you sure you want to perform an Experiment reset?")) return false;
    }
    // Check Requirements
    const labLevel = getLabLevel ? getLabLevel() : bnZero();
...
function startTsunamiSequence() {
    ...
    const controls = playTsunamiSequence(
        overlay,
        60000,
        () => {
...
export function performSurgeReset() {
    ...
    if (settingsManager.get("surge_confirmation")) {
        if (!window.confirm("Are you sure you want to perform a Surge reset?")) return false;
    }
    ...
    if (isInsufficient && settingsManager.get("insufficient_waves_confirmation")) {
        if (
            !window.confirm(
                "Are you sure you want to perform a Surge reset? You don't have enough pending Waves to reach the next Surge.",
            )
        ) {
            return false;
        }
        ...
        try {
            window.dispatchEvent(
                new CustomEvent("level:change", {
                    detail: {
                        prefix: "waves",
                        level: BigNum.fromAny(barLevel),
                        isUnlocked: isSurgeUnlocked(),
                        requirement: getSurgeRequirement(barLevel),
                    },
                }),
            );
        } catch {}
...
function formatBn(value, isSurge = false) {
    if (
        value === Infinity ||
        (value && (value === "Infinity" || (typeof value.isInfinite === "function" && value.isInfinite())))
    ) {
        if (isSurge) {
...
function generateDnaSvgDataUri() {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        paths.push(
            `<line x1="${x.toFixed(1)}" y1="${y1.toFixed(1)}" x2="${x.toFixed(1)}" y2="${y2.toFixed(1)}" stroke="#ffffff" stroke-width="2" />`,
        );
    }
...
function updateSurgeCard() {
    ...
    if (el.header) {
        ...
        setHtmlOrText(el.headerVal, sLevel);
    }
    if (el.barText)
        setHtmlOrText(
            el.barText,
    ...
    if (el.milestones) {
        const visible = getVisibleMilestones(barLevel, {
            ...
            pendingCrystals: getPendingCrystals(),
        });
        ...
        visible.forEach((m, i) => {
            ...
            let desc = m.description
                .map((d) => {
                    ...
                    if (
                        d.includes("Can't generate") ||
                        d.includes("Current bonus:") ||
                        ...
                        d.includes("Current Crystals/sec:")
                    ) {
            ...
            if (
                ...
                isSurge8 &&
                effectiveNerf < 1 &&
                (NERFED_SURGE_MILESTONE_IDS.includes(m.id) || (m.id === 36 && isPpSystemUnlocked()))
            ) {
                isNerfed = true;
            }
        ...
        if (e && e.detail && e.detail.level !== undefined) {
            ...
            if (is8 && slot != null) {
                ...
                try {
                    ...
                    lsRemoveItem(`ccc:tsunami:labPending:${slot}`);
                } catch {}
                window.dispatchEvent(new CustomEvent("unlock:change", { detail: { key: "lab", state: false, slot } }));
                window.dispatchEvent(
                    new CustomEvent("unlock:change", { detail: { key: "tsunami", state: false, slot } }),
                );
            }
            refreshNodesState();
