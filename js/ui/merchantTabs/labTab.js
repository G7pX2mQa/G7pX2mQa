...
export function setLabIntroSeen(value) {
    ...
    try {
        lsSetItem(LAB_INTRO_SEEN_KEY(slot), normalized ? "1" : "0");
    } catch {}
}
...
export function setLabLevel(value) {
    ...
    try {
        ...
        lsSetItem(LAB_LEVEL_KEY(slot), valBn.toStorage());
...
export function getLabCost(level) {
    ...
    try {
        if (lvl.cmp(1e15) < 0) {
            const lvlNum = lvl.inf || lvl.e >= BigNum.DEFAULT_PRECISION ? Infinity : Number(lvl.toPlainIntegerString());
...
export function getRpMultBase() {
    ...
    if (isSurgeActive(12)) {
        ...
        if (isSurgeActive(400)) {
            let surge400Mult = 34.89 * effectiveNerf;
...
class LabSystem {
    ...
    setupInput() {
        ...
        this.addBind(window, "keydown", (e) => {
            if (this.keys.hasOwnProperty(e.code)) this.keys[e.code] = true;
        });
        this.addBind(window, "keyup", (e) => {
    ...
    update(dt) {
        ...
        if (
            !this.lastRenderedLevel ||
            currentLevel.cmp(this.lastRenderedLevel) !== 0 ||
            !this.lastCost ||
            cost.cmp(this.lastCost) !== 0
        ) {
    ...
    render() {
        ...
        for (const node of visibleNodes) {
            ...
            if (
                cx + baseScreenSize / 2 < 0 ||
                cx - baseScreenSize / 2 > w ||
                cy + baseScreenSize * bottomBound < 0 ||
                cy - baseScreenSize / 2 > h
            ) {
                continue;
            }
            // Draw Base
            ...
            if (hasBar) {
                ...
                if (isMaxed) {
                    ...
                    if (
                        req.isInfinite?.() ||
                        (typeof req.cmp === "function" && req.cmp(BigNum.fromAny("Infinity")) === 0)
                    ) {
                        progress = 0;
                    } else {
                ...
                const textY =
                    barY + barHeight / 2 + (metrics.actualBoundingBoxAscent - metrics.actualBoundingBoxDescent) / 2;
    ...
    openNodeOverlay(id) {
        ...
        this.overlayProgress.append(
            this.overlayProgressText,
            this.overlayProgressCurrent,
            this.overlayProgressSeparator,
            this.overlayProgressMax,
        );
    ...
    updateNodeOverlay() {
        ...
        if (isMaxed) {
            ...
            setHtmlOrText(
                this.overlayLevel,
                `Level ${formatNumber(BigNum.fromAny(level))} / ${formatNumber(BigNum.fromAny(node.maxLevel))} ${statusText}`,
            );
            ...
            setHtmlOrText(
                this.overlayLevel,
                `Level ${formatNumber(BigNum.fromAny(level))} / ${formatNumber(BigNum.fromAny(node.maxLevel))}`,
            );
        ...
        if (e.touches.length === 0) {
            if (this.isDragging) {
                const dist = Math.sqrt(
                    Math.pow(this.lastTouch.x - this.dragStart.x, 2) + Math.pow(this.lastTouch.y - this.dragStart.y, 2),
                );
                if (dist < 10) {
