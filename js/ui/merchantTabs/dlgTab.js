...
export function isLabUnlockedLocal() {
    ...
    if (cachedLabUnlockedLocalStates[slot] !== undefined && cachedLabUnlockedLocalStates[slot] !== null)
        return cachedLabUnlockedLocalStates[slot];
    try {
...
function bigNumToSafeInteger(value) {
    if (value && typeof value === "object") {
        if (typeof value.toPlainIntegerString === "function") {
            try {
                const plain =
                    value.inf || value.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : value.toPlainIntegerString();
                if (plain != null) {
...
function resolveDialogueLock(meta, progress) {
    ...
    info.ariaLabel =
        rawObj?.ariaLabel ?? (status === "mysterious" ? "Hidden merchant dialogue" : "Locked merchant dialogue");
    return info;
}
        ...
        try {
            ...
            if (stored) {
                ...
                if (getActiveSlot() != null) {
                    lsSetItem(`ccc:merchant:initialChoice:${getActiveSlot() ?? "default"}`, "3");
                }
            }
        } catch {}
        if (initialChoice === 1) {
            if (script.nodes.c0 && script.nodes.c0.options[1]) {
                script.nodes.c0.options[1].label =
                    "You didn't answer my <i>\"Who are you?\"</i> question like I'd hoped.";
            }
            if (script.nodes.m1a) {
            ...
            if (script.nodes.c0 && script.nodes.c0.options[1]) {
                script.nodes.c0.options[1].label =
                    "You didn't answer my <i>\"Where am I?\"</i> question like I'd hoped.";
            }
            if (script.nodes.m1a) {
                script.nodes.m1a.say = "Yes I did.";
                script.nodes.m1a.next = "c1a";
...
function playDialogueExplosion() {
    ...
    class Particle {
        constructor(x, y, isLong, customAngle = null, customSpeed = null, customSize = null) {
            ...
            this.size =
                customSize !== null ? customSize : isLong ? Math.random() * 300 + 100 : Math.random() * 150 + 50;
...
export function startBossFightSequence() {
    ...
    setTimeout(() => {
        ...
        playSecretDlgBossFightSequence(
            visualsContainer,
            () => {
    ...
    const engine = new DialogueEngine({
        ...
        onChoice: (nodeId, opt) => {
            if (nodeId === "c1") {
                ...
                try {
                    if (getActiveSlot() != null) {
                        lsSetItem(`ccc:merchant:initialChoice:${getActiveSlot() ?? "default"}`, choice.toString());
                    }
                } catch {}
            }
        },
        onEnd: () => {
            try {
                if (getActiveSlot() != null) {
                    lsSetItem(sk(MERCHANT_MET_KEY_BASE), "1");
                }
            } catch {}
            try {
...
export function openMerchant() {
    ...
    if (slot != null) {
        ...
        if (lsGetItem(pendingKey) === "1") {
            ...
            try {
                lsSetItem(LAB_UNLOCK_KEY(slot), "1");
            } catch {}
            setLabIntroSeen(false);
            syncLabTabUnlockState();
            try {
                window.dispatchEvent(new CustomEvent("unlock:change", { detail: { key: "lab", slot } }));
            } catch {}
    ...
    if (forcedDialogueTab) {
        ...
        try {
            if (slot != null) {
                lsSetItem(`ccc:delveTab:${merchantOverlayEl.id}:${slot}`, "dialogue");
            }
        } catch {}
    }
...
function selectMerchantTab(key) {
    ...
    if (key === "workshop") {
        try {
            initWorkshopTab(merchantTabs.panels["workshop"]);
        } catch {}
    }
    if (key === "warp") {
        try {
            initWarpTab(merchantTabs.panels["warp"]);
        } catch {}
        try {
            updateWarpTab();
        } catch {}
    ...
    if (key === "lab") {
        try {
            initLabTab(merchantTabs.panels["lab"]);
        } catch {}
    ...
    if (key === "flow") {
        try {
            initFlowTab(merchantTabs.panels["flow"]);
        } catch {}
        try {
            updateFlowTab();
        } catch {}
    ...
    try {
        if (getActiveSlot() != null) {
            lsSetItem(sk(MERCHANT_TAB_KEY_BASE), key);
        }
    } catch {}
}
...
export function runLabIntroDialogue() {
    ...
    const script = {
        ...
        nodes: {
            n1: {
                type: "line",
                say: "Hey, <span style=\"color:#00e5ff\">Player</span>, welcome to the Lab. I'll keep it brief: Research nodes, increase your Lab Level to research nodes faster, and you'll be earning tons of Coins. Your Surge Milestones were temporarily sacrificed to the Tsunami but that's not important, get to researching!",
                next: "c1",
            },
            c1: {
                type: "choice",
