...
export function setNodeLocked(id, locked) {
    const slot = getActiveSlot();
    if (slot == null) return;
    lsSetItem(MAP_NODE_LOCKED_KEY(id, slot), locked ? "1" : "0");
}
    ...
    if (shouldRestart) {
        if (typeof window !== "undefined") {
            ...
            if (window.spawner && typeof window.spawner.start === "function") {
                ...
                if (
                    window.currentArea === AREAS.STARTER_COVE &&
                    typeof window.spawner.playEntranceWave === "function"
                ) {
...
export function ensureMapOverlay(unlockedNodeId = null) {
    ...
    setupDragToClose(
        grabber,
        sheet,
        () => isMapOverlayOpen && !window.__mapSequenceActive,
        () => {
    ...
    nodes.forEach((node) => {
        const isSequenceTarget =
            node.id === unlockedNodeId || (window.__mapSequenceActive && node.id === window.__mapSequenceTarget);
        ...
        pinBtn.style.marginTop = "clamp(3px, 0.6vw, 8px)";
        ...
        btn.onclick = () => {
            ...
            if (window.spawner && typeof window.spawner.hasBigCoins === "function" && window.spawner.hasBigCoins()) {
                const message =
                    typeof window.spawner.getBigCoinWarningMessage === "function"
    ...
    nodes.forEach((node) => {
        if (node.previousNodeId) {
            ...
            if (prevNode) {
                ...
                const isSequenceTarget =
                    node.id === unlockedNodeId ||
                    (window.__mapSequenceActive && node.id === window.__mapSequenceTarget);
                const isLocked = isSequenceTarget ? true : isNodeLocked(node.id, node.defaultLocked);
...
export function refreshNodesState(unlockedNodeId = null) {
    ...
    btns.forEach((btn) => {
        ...
        const isSequenceTarget =
            id === unlockedNodeId || (window.__mapSequenceActive && id === window.__mapSequenceTarget);
