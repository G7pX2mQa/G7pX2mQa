...
export class DialogueEngine {
    ...
    async goto(id) {
        ...
        if (node.type === "line") {
            ...
            await typeText(
                this.textEl,
                isJeff && node.sayJeff ? node.sayJeff : node.say,
                node.msPerChar ?? 30,
                this.skipTargets,
    ...
    _renderChoices(options, prepare = false) {
        ...
        for (const opt of options) {
            ...
            const unbind = bindRapidActivation(
                btn,
                async (event) => {
...
export function typeText(el, full, msPerChar = 30, skipTargets = [], pauseMultiplier = 14) {
    return new Promise((resolve) => {
        ...
        const tick = (currentTime) => {
            ...
            while (
                (timeAccumulator >= msPerChar || (segments[segIndex] && segments[segIndex].type === "tag")) &&
                !skipping
            ) {
                        ...
                        allTabs.forEach((tab) => {
                            tab.addEventListener("click", () => {
                                ...
                                if (targetTab && !tab.disabled && !tab.classList.contains("is-locked")) {
                                    try {
                                        lsSetItem(currentTabKey, targetTab);
                                    } catch {}
                                }
                                ...
                                requestAnimationFrame(() => {
                                    requestAnimationFrame(() => {
                                        ...
                                        if (
                                            scroller &&
                                            scroller.__customScroll &&
                                            typeof scroller.__customScroll.updateAll === "function"
                                        ) {
        ...
        if (/^[0-9]$/.test(e.key)) {
            ...
            import("../util/storage.js").then(({ getActiveSlot }) => {
                ...
                const unlockedTabs = tabs.filter(
                    (t) => !t.disabled && !t.classList.contains("is-locked") && t.style.display !== "none",
                );
