...
const HELP_ENTRIES = [
  ...
  },
  {
    ...
    nerdModeText: `<div style="margin-bottom:12px;"><strong>Milestone-type Upgrade Scaling Formula</strong><br><code>CostRatio = (1.50 + 0.1 * Evolutions) * Harshness</code><br>Where <code>Evolutions = Floor(Level / 1000)</code> and <code>Harshness</code> is an optional value to make scaling harsher for any given upgrade, defaulting to 1.<br>After 1e6 Evolutions (Level 1e9), a double-exponential softcap penalty scales exponentially: <code>ratio = 10 ^ (5 * e^(1.698e-7 * (Evolutions - 1e6))) * Harshness</code>.</div><div style="margin-bottom:12px;"><strong>Forge Base Gold Gain Formula</strong><br><code>Base Gold = Floor(10 * CoinFactor * LevelMultiplier * BonusFactor)</code><br>Where <code>CoinFactor = 2 ^ Max(0, log10(Coins) - 5)</code>, <code>LevelMultiplier = 1.4 ^ Max(0, (XPLevel - 30) / 5)</code>, and <code>BonusFactor = 1.15 ^ Floor(Max(0, log10(Coins) - 5))</code>.</div><div style="margin-bottom:12px;"><strong>Mutation Scaling Formula</strong><br>Mutations 0-9: <code>log10(Requirement) = Poly * Factor</code><br>Where <code>Poly = -0.0022 * m^2 + 0.2045 * m + 2.0168</code> and <code>m = MutationLevel + 1</code>.<br>Mutations 10-49 follow a harsher formula: <code>log10(Requirement) = 3.83 * (1.3444 ^ (Level - 9))</code>.<br>Mutations 50-99 follow an even harsher formula: <code>log10(Requirement) = 10 ^ (A * x^2 + B)</code><br>Where <code>x = Level - 49</code>, <code>A = 297/2499</code>, and <code>B = 6 - A</code>.<br>There are also breakpoints at Mutation 50 and Mutation 100 where I explicitly set the MP requirement to a flat number.<br>Also note that I may slightly change the Mutation formula whenever I feel like it for various reasons.</div><div><strong>Mutation MP Multiplier Reward</strong><br><code>Total Coins & XP Boost = 2 ^ MutationLevel</code>.</div>`,
    themeClass: "is-forge",
    isVisible: () => {
  ...
  },
  {
    ...
    nerdModeText: `<div style="margin-bottom:12px;"><strong>Infuse Base Magic Reward Formula</strong><br><code>Base Magic = Floor(10 ^ (0.811 + C * log10(1.5) + Floor(C) * log10(1.03) + Max(0, log10(CumulativeMP) - 4) * log10(2)))</code><br>Where <code>C = Max(0, log10(Coins) - 12)</code>.</div><div><strong>Workshop Level Cost Scaling Formula</strong><br>Level 1 to 1e6: <code>Cost = 10 ^ (12 + Level)</code><br>Level 1e6 to 1e9: <code>Cost = 10 ^ (1.00012e6 + (Level - 1e6) + Integral((Level - 1e6), 0.001))</code> and <code>Integral(x, k) = ((1 + k*x) * ln(1 + k*x) - k*x) / (k * ln(10))</code><br>Level 1e9 to 1e12: <code>Cost = 10 ^ (1.00000012e9 + (Level - 1e9) + (Integral((Level - 1e6), 0.001) - Integral(9.99e8, 0.001)) + 0.5 * (log10(1.00001) / 1000) * (Level - 1e9)^2)</code></code><br>Beyond Level 1e12: <code>Cost = 10 ^ (1.00000000012e12 * e^(2.3e-10 * (Level - 1T)))</code>.</div>`,
