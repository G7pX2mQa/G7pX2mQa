...
const HELP_ENTRIES = [
  ...
  },
  {
    ...
    nerdModeText: `<div style="margin-bottom:12px;"><strong>Infuse Base Magic Reward Formula</strong><br><code>Base Magic = Floor(10 ^ (0.811 + C * log10(1.5) + Floor(C) * log10(1.03) + M * log10(2)))</code><br>Where <code>C = Max(0, log10(Coins) - 12)</code> and <code>M = Max(0, log10(CumulativeMP) - 4)</code>.</div><div><strong>Workshop Level Cost Scaling Formula</strong><br>Level 1 to 1e6: <code>Cost = 10 ^ (12 + Level)</code><br>Level 1e6 to 1e9: <code>Cost = 10 ^ (1.00012e6 + U + Integral(U, 0.001))</code> where <code>U = Level - 1e6</code> and <code>Integral(x, k) = ((1 + k*x) * ln(1 + k*x) - k*x) / (k * ln(10))</code><br>Level 1e9 to 1e12: <code>Cost = 10 ^ (1.00000012e9 + V + (Integral(U, 0.001) - Integral(9.99e8, 0.001)) + 0.5 * (log10(1.00001) / 1000) * V^2)</code> where <code>V = Level - 1B</code><br>Beyond Level 1e12: <code>Cost = 10 ^ (1.00000000012e12 * e^(2.3e-10 * (Level - 1T)))</code>.</div>`,
