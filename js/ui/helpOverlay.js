...
const HELP_ENTRIES = [
  ...
  },
  {
    ...
    nerdModeText: `<div style="margin-bottom:12px;"><strong>Infuse Magic Reward Formula</strong><br><code>Base = Floor(10 ^ (0.811 + C * log10(1.5) + Floor(C) * log10(1.03) + M * log10(2)))</code><br>Where <code>C = Max(0, log10(Coins) - 12)</code> and <code>M = Max(0, log10(CumulativeMP) - 4)</code>.</div><div><strong>Workshop Level Cost Scaling Formula</strong><br>Level 1 to 1 Million: <code>Cost = 10 ^ (12 + Level * log10(10))</code> (Base cost is 1 Trillion).<br>Level 1 Million to 1 Billion: Transitions to a softer polynomial scaling utilizing logarithmic integrals with <code>k = 0.001</code>.<br>Level 1 Billion to 1 Trillion: Introduces quadratic terms to the prior scaling formula.<br>Beyond Level 1 Trillion: Extreme exponential explosion where <code>scalingExp = 2.3e-10 * (Level - 1T)</code>.</div>`,
