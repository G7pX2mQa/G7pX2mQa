...
function initDeferredFlush() {
    ...
    try {
        window.addEventListener("beforeunload", flushDeferredWrites);
    } catch {}
    try {
        window.addEventListener("pagehide", flushDeferredWrites);
    } catch {}
    try {
...
export function batchUpgradeOperations(fn) {
    ...
    try {
        ...
        pendingUpgradeSaves.forEach(({ areaKey, upgId, state, slot }) => {
            try {
                saveUpgradeState(areaKey, upgId, state, slot);
            } catch (e) {
                console.warn("Deferred upgrade save failed", e);
            }
        });
...
const SPECIAL_LOCK_STATE_TIES = new Set([
	...
	UPGRADE_TIES.UNLOCK_COMPRESS,
    UPGRADE_TIES.UNLOCK_COLLAPSE,
...
export const E = {
    addPctPerLevel(p) {
        ...
        return (level) => {
            ...
            try {
                return BN.fromInt(1).add(L.mulDecimal(pStr));
            } catch {
    ...
    addFlatPerLevel(x) {
        ...
        return (level) => {
            ...
            try {
                return BN.fromInt(1).add(L.mulDecimal(xStr));
            } catch {
                return 1;
            }
        };
...
export function determineLockState(ctx) {
    ...
    if (tieKey === UPGRADE_TIES.UNLOCK_FORGE) {
        ...
        try {
            ...
            xp31 = levelBigNumToNumber(xpBn) >= 31;
        } catch {}
        if (!xp31) {
    ...
    if (tieKey === UPGRADE_TIES.UNLOCK_SURGE) {
        ...
        try {
            const xpBn = currentXpLevelBigNum();
            xp201 = levelBigNumToNumber(xpBn) >= 201;
        } catch {}
    ...
    if (INFUSE_PLACEHOLDER_TIES.has(tieKey)) {
        ...
        try {
            isInfUnlocked = isInfuseUnlocked?.();
        } catch {}
    ...
    try {
        isForgUnlocked = isForgeUnlocked?.();
    } catch {}
...
export function normalizedUpgradeLevel(levelValue) {
    ...
    if (levelValue instanceof BigNum) {
        try {
            const plain =
                levelValue.inf || levelValue.e >= BigNum.DEFAULT_PRECISION
                    ? "Infinity"
                    : levelValue.toPlainIntegerString?.();
...
export function ensureLevelBigNum(value) {
    try {
        ...
        const plain = bn.inf || bn.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : bn.toPlainIntegerString?.();
...
function plainLevelDelta(nextLevelBn, prevLevelBn) {
    ...
    try {
        ...
        const nextPlain = next.inf || next.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : next.toPlainIntegerString?.();
        const prevPlain = prev.inf || prev.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : prev.toPlainIntegerString?.();
...
export function computeDefaultUpgradeCost(baseCost, level, upgType = "NM") {
    ...
    let ratioLog10 = preset && preset.ratioLog10 != null ? preset.ratioLog10 : Math.log10(Math.max(ratio, 1));
    const scaling = {
        baseBn,
    ...
    STANDARD(upg) {
        ...
        if (upgType === "HM") {
            ...
            if (evol > evolThreshold) {
                ...
                return (1.5 + base_scaling + extra_scaling) * harshness;
            }
            return (1.5 + 0.1 * evol) * harshness;
        }
        return 1.2 * harshness;
    },
    HM(upg) {
        const evol = activeEvolutionsForUpgrade(upg);
        const harshness = upg && upg.scalingHarshness && upg.scalingHarshness > 0 ? upg.scalingHarshness : 1;
...
function hmMilestoneHits(levelBn, milestoneLevel) {
    ...
    try {
        const plain =
            levelBn.inf || levelBn.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : levelBn.toPlainIntegerString?.();
...
function hmMilestoneMultiplier(multiplier, hits) {
    ...
    let base;
    try {
        base = BigNum.fromAny(multiplier ?? 1);
    } catch {
        base = BigNum.fromInt(1);
    }
    ...
    const multiply = (a, b) => {
        try {
            return a.mulBigNumInteger(b);
        } catch {}
        try {
            return a.mulDecimal(b.toScientific?.(12) ?? String(multiplier ?? "1"), BigNum.DEFAULT_PRECISION);
        } catch {}
        try {
            return b.mulDecimal(a.toScientific?.(12) ?? String(multiplier ?? "1"), BigNum.DEFAULT_PRECISION);
        } catch {}
        return BigNum.fromAny("Infinity");
    };
...
export function safeMultiplyBigNum(base, factor) {
    ...
    try {
        return out.mulDecimal(f.toScientific?.(12) ?? String(factor ?? "1"), BigNum.DEFAULT_PRECISION);
    } catch {}
    return out;
}
...
function ensureUpgradeScaling(upg) {
    ...
    try {
        ...
        if (!(ratio > 1)) {
            ...
            var ratioMinus1Log =
                ratio === Infinity && Number.isFinite(ratioLog10) ? ratioLog10 : Math.log10(ratioMinus1);
        }
...
function calculateBulkPurchase(upg, startLevel, walletBn, maxLevels = MAX_LEVEL_DELTA, options = {}) {
    ...
    if (!scaling) {
        if (typeof upg.costAtLevel === "function") {
            ...
            let capLimit = Number.isFinite(upg.lvlCap)
                ? Math.max(0, Math.floor(upg.lvlCap) - startLvlNum)
                : MAX_LEVEL_DELTA_LIMIT;
            let roomLimit =
                typeof maxLevels === "number" ? Math.floor(maxLevels) : Math.floor(levelBigNumToNumber(maxLevels));
            ...
            return {
                count: BigNum.fromInt(count),
                spent: spent,
                nextPrice: BigNum.fromAny(upg.costAtLevel(currentLvl)),
                numericCount: count,
            };
        }
    ...
    const maxLevelsNum = typeof maxLevels === "number" ? maxLevels : levelBigNumToNumber(maxLevels);
    ...
    if (Number.isFinite(startLevelNum) && remainingToHundred > 0) {
        ...
        if (opts.fastOnly && scaling && Number.isFinite(scaling.ratioLog10) && Number.isFinite(scaling.baseLog10)) {
            ...
            if (walletLog > maxCostLog + 5) {
                ...
                const tail = calculateBulkPurchase(upg, nextStartLevel, walletBn, room - limit, options);
        ...
        if (count < limit || reachedCap || room <= count) {
            const nextPrice = reachedCap ? zero : BigNum.fromAny(upg.costAtLevel(nextLevel));
            return { count: countBn, spent, nextPrice, numericCount: count };
        }

        const nextStartLevel = (() => {
            try {
                return toUpgradeBigNum(startLevel ?? startLevelNum, startLevelNum).add(countBn);
            } catch {
                return startLevelNum + count;
            }
        })();
        const tail = calculateBulkPurchase(upg, nextStartLevel, walletBn.sub(spent), room - count, options);
        const tailCount = tail?.count instanceof BigNum ? tail.count : countToBigNum(tail?.numericCount ?? 0);
    ...
    if (isConstantCost) {
        ...
        const { count, countBn, spent, nextPrice } = estimateFlatBulk(firstPrice, walletBn, roomBn);
        return {
            count: countBn,
            spent,
            nextPrice,
...
export function estimateFlatBulk(priceBn, walletBn, roomBn) {
    ...
    const wPlain =
        walletBn.inf || walletBn.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : walletBn.toPlainIntegerString?.();
    const pPlain = priceBn.inf || priceBn.e >= BigNum.DEFAULT_PRECISION ? "Infinity" : priceBn.toPlainIntegerString?.();
...
function emitUpgradeLevelChange(upg, prevLevelNum, prevLevelBn, nextLevelNum, nextLevelBn) {
    ...
    const payload = {
        upgrade: upg,
        oldLevel: Number.isFinite(prevLevelNum) ? prevLevelNum : levelBigNumToNumber(oldBn),
        newLevel: Number.isFinite(nextLevelNum) ? nextLevelNum : levelBigNumToNumber(newBn),
        oldLevelBn: oldBn,
        newLevelBn: newBn,
    };
    try {
...
for (const upg of REGISTRY) {
    ...
    if (upg._baseEffectVal) {
        if (upg._costScaling) {
            ...
            upg.nextCostAfter = function (_, nextLevel) {
                return costAtLevelUsingScaling(this, nextLevel);
            };
        }
...
function migrateLegacyStorage(areaKey, slot) {
    ...
    try {
        lsRemoveItem(legacyKey);
        lsRemoveItem(`${legacyKey}:backup`);
    } catch {}
}
...
function ensureUpgradeState(areaKey, upgId) {
    ...
    if (rec.lvl !== prevLvlStorage || rec.nextCost !== prevNextCost || rec.nextCostLvl !== prevNextCostLvl) {
        recNeedsSave = true;
    }
...
function computeUpgradeLockStateFor(areaKey, upg) {
    ...
    if (revealKey) {
        ...
        if (isUpgradePermanentlyUnlocked(areaKey, upg, slot)) {
            ...
            try {
                xpReached31 = levelBigNumToNumber(currentXpLevelBigNum()) >= 31;
            } catch {}
            let xpReached101 = false;
            try {
                xpReached101 = levelBigNumToNumber(currentXpLevelBigNum()) >= 101;
            } catch {}
        ...
        const applyStoredMysterious = () => {
            ...
            const reasonText =
                upg?.revealRequirement || state.reason || state.descOverride || "This upgrade is currently hidden.";
        ...
        try {
            xpReached31Now = levelBigNumToNumber(currentXpLevelBigNum()) >= 31;
        } catch {}
        let xpReached101Now = false;
        try {
            xpReached101Now = levelBigNumToNumber(currentXpLevelBigNum()) >= 101;
        } catch {}
...
export function isHmReadyToEvolve(upg, lvlBn, evolutions = null) {
    ...
    const safeEvol =
        Number.isFinite(evolutions) || evolutions === Infinity || evolutions === "Infinity"
            ? evolutions
...
function markEvolvedAnyUpgrade(slot = getActiveSlot()) {
    ...
    if (typeof localStorage === "undefined") return;
    try {
        lsSetItem(`ccc:evolvedAnyUpgrade:${slotKey}`, "1");
        if (typeof window !== "undefined") {
...
export function buyCheap(areaKey, upgId) {
    ...
    const walletHandle = bank[upg.costType];
    const wallet =
        walletHandle?.value instanceof BigNum ? walletHandle.value.clone() : BigNum.fromAny(walletHandle?.value ?? 0);
