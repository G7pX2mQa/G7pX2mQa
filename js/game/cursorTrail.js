...
if (typeof window !== "undefined" && !window.__cursorTrailGlobalSetup) {
    window.__cursorTrailGlobalSetup = true;
    window.globalLastMouseX = window.innerWidth / 2;
    window.globalLastMouseY = window.innerHeight / 2;
    window.globalMouseInside = true;
    
    window.addEventListener("pointermove", (e) => {
        window.globalLastMouseX = e.clientX;
        window.globalLastMouseY = e.clientY;
        window.globalMouseInside = true;
...
export function createCursorTrail(playfield, options = {}) {
    ...
    const forceTrail = isBossFight || options.initInCenter;

    if (forceTrail && typeof window !== "undefined") {
        pointerInside = true;
        if (window.globalMouseInside) {
            lastSpawnX = window.globalLastMouseX;
            lastSpawnY = window.globalLastMouseY;
        } else {
            lastSpawnX = window.innerWidth / 2;
            lastSpawnY = window.innerHeight / 2 + (isBossFight ? 50 : 0);
        }
    }
    ...
    const spawn = (x, y, isIdle = false) => {
        if (
            freeCount <= 0 ||
            !settingsManager.get("cursor_trail") ||
            (typeof window !== "undefined" && window.__hideCursorTrail)
        )
            return;
    ...
    const loop = () => {
        ...
        if (isBossFight && lastSpawnX !== null && lastSpawnY !== null) {
            document.dispatchEvent(
                new CustomEvent("boss_cursor_hit", {
                    detail: {
                        x: lastSpawnX,
                        y: lastSpawnY,
