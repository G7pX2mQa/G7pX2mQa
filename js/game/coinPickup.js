...
export function updateMutationSnapshot(state) {
    ...
    try {
        mutationCurrentLevelStr =
            state.level?.inf || state.level?.e >= BigNum.DEFAULT_PRECISION
                ? "Infinity"
                : (state.level?.toPlainIntegerString?.() ?? "0");
    } catch {
        mutationCurrentLevelStr = "0";
    }
...
function initMutationSnapshot() {
    ...
    try {
        mutationUnsub = onMutationChange((snapshot) => {
...
function calculateCoinValue(spawnLevelStr) {
    ...
    const mpGain =
        typeof isMutationUnlocked === "function" && isMutationUnlocked() ? BigNum.fromInt(1) : BigNum.fromInt(0);
...
export function initCoinPickup({
    ...
    function collectBatch(items) {
        ...
        for (const key in coinGroups) {
            ...
            if (g.valMult > 1) {
                try {
                    inc = inc.mulDecimalFloor(g.valMult);
                } catch {}
                try {
                    xpInc = xpInc.mulDecimalFloor(g.valMult);
                } catch {}
                try {
                    mpInc = mpInc.mulDecimalFloor(g.valMult);
                } catch {}
            }
        ...
        if (typeof window !== "undefined" && typeof window.currentArea !== "undefined" && window.currentArea === 1) {
            // 1 is AREAS.STARTER_COVE
            window.coinsCollected += collectedCount;
            const slot = getActiveSlot();
            if (slot != null) {
                try {
                    lsSetItem(`ccc:coinsCollected:${slot}`, String(window.coinsCollected));
                } catch {}
            }
        }
        if (typeof window !== "undefined") {
            ...
            try {
                lsSetItem("ccc:globalCoinsCollected", String(window.globalCoinsCollected));
            } catch {}
        }
        onCoinCollected();
    ...
    const destroy = () => {
        ...
        try {
            ["pointerdown", "pointermove", "pointerup", "mousemove"].forEach((evt) =>
                pf.replaceWith(pf.cloneNode(true)),
            );
        } catch {}
    };
    coinPickup = { destroy };
