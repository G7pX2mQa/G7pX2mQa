...
export const SURGE_MILESTONES = [
  ...
  },
  {
    ...
    affectedByTsunami: false,
    description: [
      "Doubles Gold value per Surge milestone after 69 (immune to exponent)"
    ]
  },
  {
    id: 33,
...
export function getVisibleMilestones(currentSurgeLevel, pendingVals = {}) {
  ...
  for (const m of SURGE_MILESTONES) {
    ...
    if (m.id === 32) {
      ...
      if (currentLevel >= 70) {
        const mult = getSurge70Multiplier();
        let formattedBonus = "0";
        if (mult.isInfinite?.() || currentLevelBN.isInfinite?.()) {
            formattedBonus = "Infinity";
        } else {
    ...
    if (m.id === 33) {
      ...
      if (currentLevel >= 80) {
        const mult = getSurge80Multiplier();
        let formattedBonus = "0";
        if (mult.isInfinite?.() || currentLevelBN.isInfinite?.()) {
            formattedBonus = "Infinity";
        } else {
