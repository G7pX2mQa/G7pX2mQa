                ...
                try {
                    ...
                    window.dispatchEvent(
                        new CustomEvent("stat:change", {
                            detail: { key: "xp", delta: detail.xpAdded, progress: detail.progress },
                        }),
                    );
                    window.dispatchEvent(
...
function ensureExactRequirementCacheUpTo(levelNum) {
    ...
    while (currentLevel < target) {
        ...
        const isInfinite =
            currentRequirement.isInfinite?.() ||
            (typeof currentRequirement.isInfinite === "function" && currentRequirement.isInfinite());
        if (isInfinite) {
...
function approximateRequirementFromLevel(levelBn) {
    ...
    if (!bigNumIsZero(deltaBonus)) {
        totalLog =
            totalLog.add?.(deltaBonus.mulDecimal(LOG_DECADE_BONUS_DECIMAL, BigNum.DEFAULT_PRECISION)) ?? totalLog;
    }
    ...
    if (levelBn.cmp?.(softcapStartBn) > 0) {
        ...
        let softcapDeltaNum;
        try {
            softcapDeltaNum = bigNumIsInfinite(softcapDeltaBn)
                ? Infinity
                : softcapDeltaBn.sig * Math.pow(10, softcapDeltaBn.e);
        } catch {
    ...
    if (!Number.isFinite(logNumber) || logNumber >= 1.7976931348623157e308) {
        return infinityRequirementBn.clone?.() ?? infinityRequirementBn;
    }
...
function xpRequirementForXpLevel(xpLevelInput) {
    ...
    try {
        xpLvlBn =
            xpLevelInput instanceof BigNum
    ...
    try {
        levelPlain =
            xpLvlBn.inf || xpLvlBn.e >= BigNum.DEFAULT_PRECISION
                ? "Infinity"
                : (xpLvlBn.toPlainIntegerString?.() ?? xpLvlBn.toString?.() ?? "0");
    } catch {
        levelPlain = "0";
    }
    ...
    const approxIsInf =
        approximate.isInfinite?.() || (typeof approximate.isInfinite === "function" && approximate.isInfinite());
    if (approxIsInf) {
        return infinityRequirementBn.clone?.() ?? infinityRequirementBn;
    }
    if (targetLevelInfo.num != null) {
...
function xpLevelNumInfo(xpLevelValue) {
    ...
    const levelIsInfinite =
        xpLevelValue.isInfinite?.() || (typeof xpLevelValue.isInfinite === "function" && xpLevelValue.isInfinite());
    if (levelIsInfinite) {
    ...
    try {
        plain =
            xpLevelValue.inf || xpLevelValue.e >= BigNum.DEFAULT_PRECISION
                ? "Infinity"
                : (xpLevelValue.toPlainIntegerString?.() ?? xpLevelValue.toString?.() ?? "0");
    } catch {
        plain = "0";
    }
    if (plain === "Infinity") {
...
export function syncCoinMultiplierWithXpLevel(force = false) {
    ...
    if (levelNum != null && levelNum <= EXACT_COIN_LEVEL_LIMIT) {
        ...
        let levelAdd;
        try {
            levelAdd = BigNum.fromAny(levelNum.toString());
        } catch {
    ...
    const providers =
        coinMultiplierProviders.size > 0
...
function persistState() {
    ...
    try {
        lsSetItem(KEY_UNLOCK(slot), expected.unlocked);
    } catch {}
    try {
        lsSetItem(KEY_XP_LEVEL(slot), expected.level);
    } catch {}
    try {
        lsSetItem(KEY_PROGRESS(slot), expected.progress);
    } catch {}
    const persisted = (() => {
...
function updateHud() {
    ...
    if (!xpState.unlocked) {
        ...
        if (progress) {
            ...
            setHtmlOrText(
                progress,
                `<span class="xp-progress-current">0</span><span class="xp-progress-separator">/</span><span class="xp-progress-required">${reqHtml}</span><span class="xp-progress-suffix">XP</span>`,
            );
        }
        if (bar) {
    ...
    if (progress) {
        ...
        setHtmlOrText(
            progress,
            `<span class="xp-progress-current">${currentHtml}</span><span class="xp-progress-separator">/</span><span class="xp-progress-required">${reqHtml}</span><span class="xp-progress-suffix">XP</span>`,
        );
    }
    if (bar) {
...
export function addXp(amount, { silent = false } = {}) {
    ...
    if (wasLevelInf && wasProgInf) {
        ...
        if (!inc.isZero?.()) {
            try {
                window.dispatchEvent(
                    new CustomEvent("stat:change", { detail: { key: "xp", delta: inc, progress: xpState.progress } }),
                );
            } catch {}
        }
    ...
    if (!inc.isZero?.()) {
        const providers =
            xpGainMultiplierProviders.size > 0
                ? Array.from(xpGainMultiplierProviders)
                : typeof externalXpGainMultiplierProvider === "function"
                  ? [externalXpGainMultiplierProvider]
                  : [];
        for (const provider of providers) {
            if (typeof provider !== "function") continue;
            try {
    ...
    if (!levelLocked) {
      ...
      */
        ...
        if (currentProgressLog - reqLog > 2) {
            ...
            let currentLevelNum;
            try {
                currentLevelNum = bigNumIsInfinite(baseLevelBn)
                    ? Infinity
            ...
            if (Number.isFinite(currentLevelNum)) {
                ...
                const getLogForLevel = (levelNum) => {
                    ...
                    let totalLog = levelNum * 0.04139268515822504;
...
export function getXpState() {
    if (
        xpState.xpLevel &&
        typeof xpState.xpLevel.cmp === "function" &&
        xpState.xpLevel.cmp(4500000000000) >= 0 &&
