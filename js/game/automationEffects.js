...
export function saveUcEacAccumulator() {
    try {
        ...
        const slot = getActiveSlot();
        if (slot != null) lsSetItem(`ccc:ucEacAccumulator:${slot}`, String(acc));
    } catch {}
}
            ...
            if (gain > 0) {
                ...
                if (!integerGain.isZero()) {
                    ...
                    if (!isCurrencyLocked(matKey, slot)) {
                        const finalVal =
                            bank[matKey] && bank[matKey].mult
                                ? BigNum.fromInt(1)
                                      .mulBigNumInteger(bank[matKey].mult.get())
                                      .mulBigNumInteger(integerGain)
                                : integerGain;
...
registerPassiveSystem({
    ...
    getRate: () => {
        ...
        const autoSellIsLocked =
            autoSellUpgDef && autoSellUpgDef.requiredNodeId && isNodeLocked(autoSellUpgDef.requiredNodeId, true);
        if (
            autoSellIsLocked ||
            autoSellLevel <= 0 ||
            !bank.scrap ||
            globalThis?.__cccLockedStorageKeys?.has?.("ccc:scrap")
        )
            return 0;
        const autoSellSetting = settingsManager.get("auto_sell_efficiency");
