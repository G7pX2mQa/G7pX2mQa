...
export function getEacAmountMultiplier() {
  ...
  export function updateAutomation(dt) {
    ...
    let efficiencyMult = eacEfficiency !== undefined ? (eacEfficiency / 100) : 1;
  ...
  // Auto-Sell Logic
  const autoSellEfficiency = settingsManager.get('auto_sell_efficiency');
  ...
  if (autoSellEfficiency !== 0 && !autoSellIsLocked && autoSellLevel > 0 && bank.scrap && !globalThis?.__cccLockedStorageKeys?.has?.('ccc:scrap')) {
      let eff = 0;
      ...
      const autoSellEfficiencyMult = autoSellEfficiency !== undefined ? (autoSellEfficiency / 100) : 1;
      eff *= autoSellEfficiencyMult;
