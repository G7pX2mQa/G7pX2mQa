...
export function incrementLifetimeSizeCoinsCollected(size, slot = getActiveSlot()) {
    ...
    try {
        lsSetItem(getSizeCoinsCollectedKey(size, slot), String(current + 1));
    } catch {}
}
...
export function setLifetimeUselessExperiment(slot = getActiveSlot()) {
    if (slot == null) return;
    try {
        lsSetItem(`ccc:secretAchievements:uselessExperiment:${slot}`, "1");
    } catch {}
}
...
export function setLifetimeBossBeaten(slot = getActiveSlot()) {
    if (slot == null) return;
    try {
        lsSetItem(`ccc:secretAchievements:bossBeaten:${slot}`, "1");
    } catch {}
}
...
export function setVaultSequence(seq, slot = getActiveSlot()) {
    if (slot == null) return;
    try {
        lsSetItem(`ccc:secretAchievements:vaultSequence:${slot}`, seq);
    } catch {}
}
...
const _rawSecretAchievements = [
    {
        ...
        title: "A Large One",
        get desc() {
            return `Collect a Coin of size 4 (1/${formatNumber(BigNum.fromAny(10000))} chance to spawn)`;
        },
    ...
    },
    {
        ...
        title: "A Larger One",
        get desc() {
            return `Collect a Coin of size 5 (1/${formatNumber(BigNum.fromAny(100000))} chance to spawn)`;
        },
    ...
    },
    {
        ...
        get desc() {
            return `Collect a Coin of size 6 (1/${formatNumber(BigNum.fromAny(1000000))} chance to spawn)`;
        },
    ...
    },
    {
        ...
        get desc() {
            return "Defeat the Merchant in his secret boss fight";
        },
    ...
    },
    {
        ...
        title: "Safety First",
        get desc() {
            return "Toggle the reset confirmations for Forge, Infuse, Surge, and Experiment to ON";
        },
        icon: "img/misc/safety_first.webp",
        checkCondition: (slot) => {
            return (
                settingsManager.get("forge_confirmation") &&
    ...
    },
    {
        ...
        get desc() {
            return "Perform an Experiment reset while having XP Level 0";
        },
    ...
    },
    {
        ...
        get desc() {
            return 'Configure the automation of five different currencies to a "Sort of ON" state';
        },
    ...
    },
    {
        ...
        title: "Binary Flow",
        get desc() {
            return "In the Flow tab, construct the word \"Flow\" in binary where toggling the Coin Waterwheel's Flow State represents 0, and toggling the XP Waterwheel's Flow State represents 1. Toggles must be consecutive.";
        },
...
function saveSecretAchievementState(state, slot = getActiveSlot()) {
    ...
    try {
        lsSetItem(`${SECRET_ACHIEVEMENT_STATE_KEY_BASE}:${slotKey}`, JSON.stringify(state));
    } catch {}
}
            ...
            if (achievement.checkCondition(slot)) {
                ...
                if (!achievement.notifyCondition || achievement.notifyCondition()) {
                    ...
                    window.__delayedSecretAchievementNotifications =
                        window.__delayedSecretAchievementNotifications || [];
                    window.__delayedSecretAchievementNotifications.push({
...
if (typeof window !== "undefined") {
    ...
    window.addEventListener("saveSlot:change", () => {
        secretAchievementStateCache.clear();
