...
function progressRatio(progressBn, requirement) {
    ...
    const reqIsInf = !!(
        requirement.isInfinite?.() ||
        (typeof requirement.isInfinite === "function" && requirement.isInfinite())
    );
    const progIsInf = !!(
        progressBn.isInfinite?.() ||
        (typeof progressBn.isInfinite === "function" && progressBn.isInfinite())
    );
    if (reqIsInf) {
...
function updateHud() {
    ...
    if (!ppState.unlocked) {
        ...
        if (progress) {
            ...
            setHtmlOrText(
                progress,
                `<span class="pp-progress-current">0</span><span class="pp-progress-separator">/</span><span class="pp-progress-required">${reqHtml}</span><span class="pp-progress-suffix">PP</span>`,
            );
        }
        if (bar) {
    ...
    if (progress) {
        ...
        setHtmlOrText(
            progress,
            `<span class="pp-progress-current">${currentHtml}</span><span class="pp-progress-separator">/</span><span class="pp-progress-required">${reqHtml}</span><span class="pp-progress-suffix">PP</span>`,
        );
    }
    if (bar) {
    ...
    try {
        ...
        window.dispatchEvent(
            new CustomEvent("level:change", {
                detail: {
                    prefix: "pp",
                    level: detail.ppLevel,
                    progress: detail.progress,
                    requirement: detail.requirement,
                    isUnlocked: detail.unlocked,
                    ratio: getPpProgressRatio(),
                },
            }),
        );
    } catch {}
...
export function addPp(amount, { silent = false } = {}) {
    ...
    const wasLevelInf = !!(
        ppState.ppLevel?.isInfinite?.() ||
        (typeof ppState.ppLevel?.isInfinite === "function" && ppState.ppLevel.isInfinite())
    );
    const wasProgInf = !!(
    ...
    if (wasLevelInf && wasProgInf) {
        if (!inc.isZero?.()) {
            try {
                window.dispatchEvent(
                    new CustomEvent("stat:change", { detail: { key: "pp", delta: inc, progress: ppState.progress } }),
                );
            } catch {}
        }
            ...
            try {
                ...
                window.dispatchEvent(
                    new CustomEvent("stat:change", {
                        detail: { key: "pp", delta: detail.ppAdded, progress: detail.progress },
                    }),
                );
                window.dispatchEvent(
                    new CustomEvent("level:change", {
    ...
    if (currentProgressLog - reqLog > 2) {
        let currentLevelNum;
        try {
            currentLevelNum = bigNumIsInfinite(ppState.ppLevel)
                ? Infinity
                : ppState.ppLevel.sig * Math.pow(10, ppState.ppLevel.e);
        } catch {
            currentLevelNum = 0;
        }
...
export function getPpState() {
    ...
    if (
        !levelLocked &&
        typeof ppState.ppLevel?.cmp === "function" &&
        ppState.ppLevel.cmp(4500000000000) >= 0 &&
        !ppState.ppLevel.isInfinite?.()
    ) {
...
export function unlockPpSystem() {
    ...
    const detail = getPpState();
    try {
        window.dispatchEvent(new CustomEvent("pp:unlock", { detail }));
        window.dispatchEvent(
            new CustomEvent("level:change", {
                detail: {
                    prefix: "pp",
