...
function handleExternalDpStorageChange(reason) {
    ...
    try {
        ...
        if (levelChanged) {
            try {
                dpLevelsGained = current.dpLevel.sub?.(prev.dpLevel) ?? bnZero();
            } catch {
        ...
        if (!levelChanged && progressChanged) {
            try {
                dpAdded = current.progress.sub?.(prev.progress) ?? null;
            } catch {
                dpAdded = null;
            }
        }
                ...
                try {
                    ...
                    window.dispatchEvent(
                        new CustomEvent("stat:change", {
                            detail: { key: "dp", delta: detail.dpAdded, progress: detail.progress },
                        }),
                    );
                    window.dispatchEvent(
                        new CustomEvent("level:change", {
                            detail: {
                                prefix: "dp",
                                level: detail.dpLevel,
                                progress: detail.progress,
                                requirement: detail.requirement,
                                isUnlocked: detail.unlocked,
                                ratio: getDpProgressRatio(),
                            },
                        }),
                    );
                } catch {}
...
function progressRatio(progressBn, requirement) {
    ...
    const isInfinite = (bn) =>
        !!(
            bn &&
            typeof bn === "object" &&
            (bn.isInfinite?.() || (typeof bn.isInfinite === "function" && bn.isInfinite()))
        );
...
function dpRequirementForDpLevel(dpLevelInput) {
    ...
    try {
        dpLvlBn =
            dpLevelInput instanceof BigNum
    ...
    try {
        levelPlain =
            dpLvlBn.inf || dpLvlBn.e >= BigNum.DEFAULT_PRECISION
                ? "Infinity"
                : (dpLvlBn.toPlainIntegerString?.() ?? dpLvlBn.toString?.() ?? "0");
    } catch {
        levelPlain = "0";
    }
...
function persistState() {
    ...
    try {
        lsSetItem(KEY_DP_LEVEL(slot), expected.level);
    } catch {}
    try {
...
function updateHud() {
    ...
    if (!dpState.unlocked) {
        ...
        if (progress) {
            ...
            setHtmlOrText(
                progress,
                `<span class="dp-progress-current">0</span><span class="dp-progress-separator">/</span><span class="dp-progress-required">${reqHtml}</span><span class="dp-progress-suffix">DP</span>`,
            );
        }
        if (bar) {
    ...
    if (progress) {
        ...
        setHtmlOrText(
            progress,
            `<span class="dp-progress-current">${currentHtml}</span><span class="dp-progress-separator">/</span><span class="dp-progress-required">${reqHtml}</span><span class="dp-progress-suffix">DP</span>`,
        );
    }
    if (bar) {
        ...
        if (isPpSystemUnlocked()) {
            ...
            if (ppLevel && !ppLevel.isZero()) {
                ...
                if (!Number.isFinite(log10ResultNum) || log10ResultNum === Infinity) {
                    ...
                    const powValBn = bigNumFromLog10(log10ResultNum).floorToInteger();
...
export function addDp(amount, { silent = false } = {}) {
    ...
    const wasLevelInf = !!(
        ...
        (typeof dpState.dpLevel?.isInfinite === "function" && dpState.dpLevel.isInfinite())
    );
    const wasProgInf = !!(
    ...
    if (wasLevelInf && wasProgInf) {
        if (!inc.isZero?.()) {
            try {
                window.dispatchEvent(
                    new CustomEvent("stat:change", { detail: { key: "dp", delta: inc, progress: dpState.progress } }),
                );
            } catch {}
        }
        updateHud();
    ...
    if (currentProgressLog - reqLog > 2) {
        let currentLevelNum;
        try {
            currentLevelNum = bigNumIsInfinite(dpState.dpLevel)
                ? Infinity
    ...
    if (!hasDoneCompressReset() && dpState.dpLevel.cmp(BigNum.fromInt(99)) >= 0) {
        if (dpState.progress.cmp(requirementBn) >= 0) {
            ...
            if (typeof window !== "undefined" && !window.__isDpCappedPreCompress) {
                ...
                const msg = `<span style="display: block; font-size: 85% !important; line-height: 1.4 !important;">Oh no! Your current pickaxe is<br>not strong enough to mine to 100m!<br>You'll need the <span class="text-prismatic-pickaxe" style="${rainbowStyle}">Prismatic Pickaxe</span><br>to dig further, which can be obtained at Surge 200.</span>`;
...
export function getDpState() {
    ...
    if (
        !levelLocked &&
        typeof dpState.dpLevel?.cmp === "function" &&
        dpState.dpLevel.cmp(4500000000000) >= 0 &&
        !dpState.dpLevel.isInfinite?.()
    ) {
        dpState.dpLevel = BigNum.fromAny("Infinity");
    }
