...
const UPGRADE_DEFINITIONS = [
  ...
  },
  {
    ...
    icon: 'img/uc_upg_icons/manual_materials_value.webp',
    lvlCap: 4,
    baseCost: 1e9999,
    costType: 'gears',
    upgType: 'NM',
    ...
    costAtLevel(level) {
      const lvl = Math.max(0, Math.floor(Number(level) || 0));
      return BigNum.fromAny('1e9999').mulBigNumInteger(E.powPerLevel(BigNum.fromAny('1e50'))(lvl));
    },
    effectSummary(level) {
      const lvl = Math.max(0, Math.floor(Number(level) || 0));
      return `Manual material value bonus: +${lvl * 100}%`;
    },
    computeLockState(ctx) {
        const sl = ctx.surgeLevel;
        let isUnlocked = false;
  ...
  },
  {
    ...
    costAtLevel(level) {
      const lvl = Math.max(0, Math.floor(Number(level) || 0));
      let ratioBn = BigNum.fromAny('1e1000');
      // powPerLevel(ratio)(lvl) equals ratio^lvl
      // since the ratio is huge, powPerLevel handles it via logs
      return BigNum.fromAny('1e9999').mulBigNumInteger(E.powPerLevel(ratioBn)(lvl));
    },
    effectSummary(level) {
      const lvl = Math.max(0, Math.floor(Number(level) || 0));
