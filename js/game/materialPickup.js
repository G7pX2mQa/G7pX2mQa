    ...
    for (const [handle, gain] of pendingMaterialGains.entries()) {
        if (gain && !gain.isZero?.()) {
            try {
                ...
                console.error("Error adding material gain", e);
            }
        }
    }
...
export function initUcPickup({
    ...
    const checkUcShopUnlock = () => {
        ...
        if (activeSlot != null) {
            ...
            if (p >= 10 && lsGetItem(SHOP_UC_UNLOCK_KEY) !== "1") {
                ...
                lsSetItem(SHOP_UC_UNLOCK_KEY, "1");
            }
        }
    };
    checkUcShopUnlock();
    ...
    function collectBatch(items) {
        ...
        if (
            ...
            currentArea !== AREAS.UNDERWATER_CAVERN
        )
            return;
        ...
        if (collectedCount > 0) {
            ...
            for (const [matType, count] of Object.entries(gains)) {
                ...
                let isLocked = false;
                try {
                    isLocked = globalThis?.__cccLockedStorageKeys?.has?.(`ccc:${matType}`);
                } catch {}
            ...
            if (
                ...
                typeof window.currentArea !== "undefined" &&
                window.currentArea === 2
            ) {
