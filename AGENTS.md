...
- When creating new keys to save to localStorage, it is VERY important that all entries start with the prefix ccc and end with the current slot number. Also don't use the native localStorage setters or removers, but use the custom functions `lsSetItem` or `lsRemoveItem`
- Never use BigInt.
